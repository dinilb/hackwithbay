...
> **Supersedes the old "Bazaar + Mandate" plan.** The Neo4j trust-graph, scoped mandates, single-use virtual cards, approval gates, and provenance paths are **cut** — they contradict the new pitch's core promise (*pay once, then buy frictionlessly*). This doc is the source of truth; `design.md`'s visual system still applies (just rename Bazaar → Mercato in copy). See §13 for what maps over from the old code.
...
| LLM calls | Butterbase AI gateway, `claude-sonnet-4-6` (ranking/agent). Use the latest capable Claude model. |
...
| Neo4j (optional) | Live at `POST https://9d7c9060.databases.neo4j.io/db/neo4j/query/v2` · basic auth `neo4j`/`$NEO4J_PASSWORD` (.env). **Not used by Mercato core.** Only a stretch idea (§12: supplier-relationship graph). |
...
|---|---|---|---|---|
| `butterbase` | http | `https://api.butterbase.ai/mcp` | header `Authorization: Bearer $BUTTERBASE_API_KEY` | Key `bb_sk_...` in `.env`. **Primary backend** for Mercato: DB, functions, auth, AI, billing, frontend deploy. |
| `rocketride` | stdio | `/Users/cs/.local/bin/rocketride-mcp` (pipx, v1.3.0) | env `ROCKETRIDE_URI=https://api.rocketride.ai` + `ROCKETRIDE_APIKEY=rr_...` | Optional engine for the search→rank→present **AI pipeline** (§8). Timeboxed/stretch. Needs Python 3.10+ (installed via pipx). |
| `neo4j` | stdio | `neo4j-mcp` (`/opt/homebrew/bin/neo4j-mcp`, v1.5.3) | env `NEO4J_URI`/`NEO4J_USERNAME`/`NEO4J_PASSWORD`/`NEO4J_DATABASE` | **Not in Mercato core.** Cursor does NOT expand `${VAR}` — inline literals in `mcp.json`. Keep connected only for the §12 stretch. |
...
- **`shop-agent`** — `POST {message, session_id}` (the "Find me X" NL task) →
  ① write `agent_events` `"searching…"` steps →
  ② call `search` →
  ③ LLM (Butterbase AI) ranks and selects the **top 4 dealer offers** for the best-matching product, each with a one-line reason →
  ④ return `{product, offers:[4], narration}`. Streams events so the UI can *visualize* the search (beat 1).
...
| 4 | *(stretch)* RocketRide fronts `shop-agent` pipeline (§8), timeboxed 15m, fallback = inline | *(stretch)* extra product coverage | endpoint live OR fallback |
| 5 | reset → **rehearse runbook ×2** → backup screen-recording → **SUBMIT** | — | submission confirmed |

## 8. RocketRide (optional pipeline engine)

`shop-agent`'s search → rank → present is a textbook AI pipeline. RocketRide can host it (per `.claude/rules/rocketride.md` — read `.rocketride/docs/` before writing any RocketRide code). Keep it a **timeboxed stretch**: the inline Butterbase-function version is the fallback and ships first.
...
**Cut order:** RocketRide → freeform tasks (chips only) → confirmation email (in-app receipt suffices) → design polish.
...
**Risks:** LLM ranking flakiness → constrain output to strict JSON (`{offers:[{offer_id, reason}]}`) and cap at 4; search returning <4 offers → seed guarantees ≥4 on the demo product; clock → cut per order above.

---

## 12. Optional stretch — supplier-relationship graph (Neo4j)

Only if the core demo is solid and time remains. A graph layer could model *supplier trust and relationships* — who fulfills for whom, distributor → reseller chains, reliability propagation — to justify *why* a dealer ranks where it does. Keep it strictly additive: it must never gate a purchase (that would break pay-once). Cut on sight of any risk to the 4 beats.
...
| `GraphCanvas.tsx`, `VirtualCard.tsx`, `ApprovalCard.tsx` | **Drop** — mandate/graph/approval era, contradicts pay-once. |
| `scripts/neo4j.mjs`, `seed-graph.mjs` | **Drop** from core (§12 stretch only). |
